...
controllers
		.controller(
				...
				function($scope, $http, $window) {
					$scope.actionname = localStorageGet(
							"editaction_actionname", false);
					var actionid = localStorageGet("editaction_actionid", false);
					$scope.theAction = new ActionVo();
					$scope.theAction.actionid = actionid;
					$scope.theAction.actionname = $scope.actionname;
					$scope.getActionPermissionList = function() {
						$(".loading-container").removeClass("loading-inactive");
						var url = baseUrl
								+ "getActionPermissionList";
					...
					$scope.addUserAuth = function(auth) {
						...
						if (isAdd) {
							...
							bootbox.confirm("确认为" + $scope.actionname + "分配"
					...
					$scope.goBack = function() {
						bootbox.setDefaults("locale", "zh_CN");
						bootbox.confirm("确认放弃所编辑接口？", function(result) {
							if (result) {
								$window.history.back();
							}
						});
					...
					$scope.saveAction = function() {
						editActionForm = $("#editActionForm").data(
								'bootstrapValidator');
						editActionForm.validate();
						if (editActionForm.isValid() == true) {
							var parmAction = $scope.theAction;
							if (parmAction.actionname == $scope.actionname) {
								parmAction.actionname = undefined;
							}
