				...
				function($scope, $http, $window) {
					...
					$scope.addUserAuth = function(auth) {
						...
						if (isAdd) {
							...
							bootbox.confirm("确认为新角色分配" + auth.authitemname
									+ "行为吗？", function(result) {
								if (result) {
					...
					$scope.goBack = function() {
						bootbox.setDefaults("locale", "zh_CN");
						bootbox.confirm("确认放弃所添加角色？", function(result) {
							if (result) {
								$window.history.back();
							}
						});
					...
					$scope.saveRole = function() {
						addRoleValidator = $("#addRoleForm").data(
								'bootstrapValidator');
						addRoleValidator.validate();
						if (addRoleValidator.isValid() == true) {
							var url = baseUrl + "addRole";
							$(".loading-container").removeClass(
									"loading-inactive");

							var data = {
								"authitem":$scope.theRole,
								"authitemList":JSON.stringify($scope.userPermissions)
							};
